...
public abstract class CanSocket<J extends Structure, M extends CanMessage<J>> implements Closeable {
	...
	protected void receiveFrom(J struct, CanInterface canIf) throws IOException {
		...
		long bytesRead = cLib.recvfrom(fd, struct.getPointer(), new NativeSize(struct.size()), 0, sockAddr, addrSize)
				.longValue();
		if (bytesRead < 0) {
			if (timeoutEnabled) {
				throw new SocketTimeoutException();
	...
	public void sendTo(CanInterface canIf, M msg) throws IOException {
		...
		long bytesWritten = cLib.sendto(fd,
				struct.getPointer(),
				new NativeSize(struct.size()),
				0,
				sockAddr,
				sockAddr.size()).longValue();
		if (bytesWritten < 0) {
